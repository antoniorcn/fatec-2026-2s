...
* **Validação de Soma dos Pesos:** Validar no `Service` se a soma do campo `peso` de todas as avaliações pertencentes a uma `TurmaDTO` totaliza exatamente `1.00` (100%) antes de salvar.
* **Gerenciamento em Cascata:** Ao remover uma `TurmaDTO`, todas as suas entidades `Avaliacao` vinculadas devem ser excluídas do banco de dados automaticamente.