...
import edu.curso.pulse.mapper.TurmaMapper
...
class TurmaController(
    ...
    private val mapper : TurmaMapper
) {
    ...
    fun getAll() : ResponseEntity<List<TurmaViewDTO>> {
        return try {
            ...
            val listaTurmaViewDTO = listaTurma.map { turma -> mapper.toDTO(turma) }
    ...
    fun add(@Valid @RequestBody turmaDTO : TurmaDTO) : ResponseEntity<TurmaViewDTO> {
        return try {
            val turma = mapper.toModel(turmaDTO)
            val turmaGravada = service.add(turma)
            val turmaGravadaViewDTO = mapper.toDTO( turmaGravada )
